...
func TestThroughput(t *testing.T) {
	...
	// 测试不同数据大小的吞吐量
	dataSizes := []int{128, 512, 1024, 2048, 4096} // bytes
	...
	for _, size := range dataSizes {
		...
		encryptionLoop:
			...
			t.Logf("数据大小: %d bytes, 总操作数: %d, 吞吐量: %.2f ops/sec, 数据速率: %.2f MB/s",
				size, count, throughput, dataRate)
		})
	}
}
